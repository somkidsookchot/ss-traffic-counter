...
cv2.createTrackbar('Count area','Settings',225,1000,nothing)
cv2.createTrackbar('Count line','Settings',250,1000,nothing)
cv2.createTrackbar('Min size','Settings',300,10000,nothing)
cv2.createTrackbar('Max size','Settings',50000,100000,nothing)

def on_disconnect(client, userdata, rc):
    client.publish("BackGate/status","Disconnected")
    if rc != 0:
        print ("Unexpected MQTT disconnection. Will auto-reconnect")

broker_address = args.mqtt
client = mqtt.Client("BackGate") #create new instance
client.connect(broker_address) #connect to broker
client.on_disconnect = on_disconnect
client.loop_start()
client.publish("BackGate/status","active")
...
if(latestDate != todayInt):
    ...
    carscrossedup = int(arr_read[0][0])  # keeps track of cars that crossed up
    carscrosseddown = int(arr_read[1][0])  # keeps track of cars that crossed down
...
while True:
    ...
    if ret:  # if there is a frame continue with code
        ...
        sl = cv2.getTrackbarPos('Count area','Settings')
        cl = cv2.getTrackbarPos('Count line','Settings')
        mn = cv2.getTrackbarPos('Min size','Settings')
        mx = cv2.getTrackbarPos('Max size','Settings')

        overlay = image.copy()
        cv2.rectangle(overlay, (0, sl), (width, 400), (0, 200, 200), -1)  # A filled rectangle
        ...
        lineypos = sl
        cv2.line(image, (0, lineypos), (width, lineypos), (0, 255, 0), 5)

        # line y position created to count contours
        lineypos2 = cl
        cv2.line(image, (0, lineypos2), (width, lineypos2), (255, 0, 0), 5)
            ...
            if curcent:  # if there is a current centroid
                ...
                if oldcent:  # checks if old centroid exists
                    ...
                    if oldcent[1] >= lineypos2 and curcent[1] <= lineypos2 and carids[
                        ...
                        # ws = create_connection("ws://192.168.1.36:1880/ws/crossedup")
                        # msg = {"up":carscrossedup}
                        # ws.send(json.dumps(msg))
                        # ws.close()

                        client.publish("BackGate/crossedup",carscrossedup)
                        client.publish("BackGate/crosseddown",carscrosseddown)
                        client.publish("BackGate/status",'active')
                    ...
                    # to count cars and that car hasn't been counted yet
                        ...
                        # ws = create_connection("ws://192.168.1.36:1880/ws/crosseddown")
                        # msg = {"down":carscrosseddown}
                        # ws.send(json.dumps(msg))
                        # ws.close()
        ...
        cv2.rectangle(image, (0, 0), (220, 70), (0, 0, 0), -1)  # background rectangle for on-screen text
        ...
        cv2.putText(image,"Date: "+str(startCount),(0, 62), cv2.FONT_HERSHEY_SIMPLEX, .4, (255, 255, 255), 1)
